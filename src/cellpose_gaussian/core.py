...
# # ---- Use it ----
# # Get image as (C, H, W)
# def extract_arr_to_df(img, img_layer="image", seg_layer="segmented_watershed"):
#     arr = get_image_CHW(img, layer=img_layer)
#     # C, H, W = arr.shape
    
#     # Your labels should be 2-D (H, W). Align scale/library with the image if needed:
#     labels = img[seg_layer]
#     # match the same selections as image (library_id/scale) if those dims exist:
#     if "library_id" in labels.dims and "library_id" in img["image"].dims:
#         labels = labels.sel(library_id=img["image"].coords["library_id"].values[0])
#     if "scale" in labels.dims and "scale" in img["image"].dims:
#         labels = labels.sel(scale=img["image"].coords["scale"].values.min())
...
#     print("Normalized image shape (C,H,W):", arr.shape)
#     print("Labels shape (H,W):", labels.shape)
...
def plot_merge_suggestions(
    ...
    merge_suggestions : pandas.DataFrame
        Output of `suggest_label_merges`.
        Must contain columns: 'label_1', 'label_2'.
    channel_to_show : int, optional
        Channel to display if `use_color=False`.
    use_color : bool, optional
        Display multichannel image if True.
    contour_color : str, optional
        Color of segmentation contours.
    suggestion_color : str, optional
        Color used for merge suggestion lines and markers.
    label_color : str, optional
        Color for numeric labels.
    linewidth : float, optional
        Width of suggestion lines.
    alpha : float, optional
        Transparency for suggestion lines.
    figsize : tuple, optional
        Figure size.
    max_pairs : int, optional
        Maximum number of merge pairs to display (for dense images).

    Notes
    -----
    - Lines connect centroids of suggested label pairs.
    - Only suggestions present in `labels` are drawn.
    """

    # ------------------------------------------------------------
    # Prepare base image (same logic as your function)
    # ------------------------------------------------------------
    if arr.ndim == 3:
        if use_color:
    ...
    # ------------------------------------------------------------
    # Overlay numeric labels for involved regions only
    # ------------------------------------------------------------
